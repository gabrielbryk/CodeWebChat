...
export class WorkspaceProvider
  ...
  private handle_file_out_preview(file_path: string): void {
    ...
    if (was_opened_from_workspace_view) {
      ...
      this.directory_selected_token_counts.delete(path.dirname(file_path))
  ...
  getTreeItem(element: FileItem): vscode.TreeItem {
    ...
    if (element.isDirectory) {
      if (total_token_count !== undefined && total_token_count > 0) {
        ...
        else if (selected_token_count === total_token_count) {
    ...
    element.description = display_description.trim()
      ? display_description.trim()
      : undefined
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isSymbolicLink()) {
          try {
            is_directory = (await fs.promises.stat(full_path)).isDirectory()
          } catch {
            continue
          } // broken symlink
        }
      ...
      // console.error(`Error calculating tokens for dir ${dir_path}:`, error)
      return 0
    }
  }
  ...
  private async _calculate_directory_selected_tokens(
    ...
    try {
      ...
      // console.error(`Error calc selected tokens for dir ${dir_path}:`, error)
      return 0
    }
    ...
    try {
      ...
      for (const entry of dir_entries) {
        ...
        if (checkbox_state === undefined) {
          const parent_checkbox_state = this.checked_items.get(
  ...
  private async update_directory_check_state(
    ...
    this.directory_selected_token_counts.delete(dir_path); // Invalidate self
    try {
      const workspace_root = this.get_workspace_root_for_file(dir_path)
      if (!workspace_root) return
      ...
      for (const entry of dir_entries) {
        ...
        this.directory_selected_token_counts.delete(full_path); // Invalidate child
      ...
      // console.error(`Error updating dir check state for ${dir_path}:`, error)
    }
  }
  ...
  private async load_all_gitignore_files(): Promise<void> {
    ...
    for (const file_uri of gitignore_files) {
      ...
      try {
        ...
        // console.error(`Error reading .gitignore ${gitignore_path}:`, error)
      }
    }
  ...
  public update_attach_open_files_setting(value: boolean): void {
    if (this.attach_open_files == value) return
    ...
    for (const file_path of checked_files) {
      try {
        if (fs.statSync(file_path).isFile()) {
          total += this.file_token_counts.has(file_path)
            ? this.file_token_counts.get(file_path)!
            : await this.calculate_file_tokens(file_path)
        }
      } catch (error) {
        // console.error(`Error accessing ${file_path} for token count:`,error)
      }
    }
    return total
  }
...
export class FileItem extends vscode.TreeItem {
  constructor(
    ...
    public selectedTokenCount?: number, // Added selectedTokenCount
    description?: string,
    public isWorkspaceRoot: boolean = false
  ) {
    super(label, collapsibleState)
    this.tooltip = this.resourceUri.fsPath // Base tooltip
    this.description = description // Initial description
